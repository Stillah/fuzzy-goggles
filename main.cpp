...
int main() {
    ...
    int n, m, numberOfUniqueWords = 0;
    cin >> n;
    string in;
    HashMap<string, int> frequencyN, frequencyM;
    vector<string> output;
    for (int i = 0; i < n; i++) cin >> in, frequencyN.put(in, 1);
    cin >> m;

    for (int i = 0; i < m; i++) {
        cin >> in;
        if (frequencyM.get(in) == 0) {
            frequencyM.put(in, 1);
            if (frequencyN.get(in) == 0) output.push_back(in), numberOfUniqueWords++;
        }
    }
